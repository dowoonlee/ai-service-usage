...
Deno.test("레인보우 배틀 파리티 (이로치 버프 + 레인보우 크리 + variant4 스킬셋) — seed 9999999", () => {
  ...
  // 팀 게이지 재캡처 — 궁극기 충전이 행동+피격+승계(ULT_CHARGE_COST=10)로 바뀌어 variant4 A측의
  // 궁 발동 타이밍이 데미지 시퀀스에 반영됨(winner/rounds/crits는 이 시드에선 동일).
  ...
  assertEq("dmg sequence", r.log.map((e) => e.damage),
    [17, 2, 11, 15, 2, 10, 11, 18, 10, 17, 11, 18, 11, 15, 19, 10, 10, 18, 10, 23, 25, 25, 10, 23, 10, 25, 11, 24, 11, 23, 23, 23, 25, 11, 23, 15, 24, 11, 24]);
});
...
Deno.test("궁극기 배틀 파리티 (variant4 팀 게이지 N=10 발동) — seed 8642097", () => {
  ...
  // 궁극기 id는 6타입 카탈로그 중 하나. 이 배틀에선 rm_rf(warrior)가 3회 발동.
  ...
  assertEq("궁극기 발동 수", ults.length, 3);
  assertEq("궁극기 종류", [...new Set(ults.map((e) => e.move))].sort(), ["rm_rf"]);
  assertEq("dmg sequence", r.log.map((e) => e.damage),
    [26, 24, 24, 25, 24, 25, 26, 80, 39, 43, 45, 28, 27, 26, 25, 39, 28, 25, 26, 70, 129]);
});
