// 게시판 조회 — `board` 엔드포인트와 `sync`가 공유한다.
//
// 원래 board/index.ts 안에 있던 조회·조립 로직을 그대로 옮긴 것이다(순수 이동).
// 익명 닉네임 시드 규칙도 함께 옮겨야 두 경로가 같은 닉네임을 낸다.
...
/** 하위 조회(글/좋아요/댓글)가 실패했을 때. 호출부가 500으로 매핑한다. */
...
/**
 * 표시 윈도우 내 게시글 + 좋아요/댓글 + 본인 cooldown을 조립해 반환한다.
 * `deviceIdLower`가 null이면 익명 조회 — isMine/likedByMe는 전부 false.
 */
export async function fetchBoard(
  db: ReturnType<typeof getDb>,
  opts: { tenant: string; deviceId: string | null; deviceIdLower: string | null },
) {
  const { tenant, deviceId, deviceIdLower } = opts;
