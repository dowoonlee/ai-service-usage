...
CREATE TABLE guilds (
    ...
    -- 가구 재배치 (길드장) — 바닥 가구 세트의 자유 배치 좌표. "setId:x:lane;…" 직렬화 문자열
    -- (빈 문자열 = 클라이언트 기본 배치). 파싱·검증은 guild-manage의 set_furniture가 담당.
    -- 멤버 자리(office_slot 포지션)는 이와 무관하게 고정 — 가구만 드래그로 움직인다.
    office_furniture TEXT NOT NULL DEFAULT '',
