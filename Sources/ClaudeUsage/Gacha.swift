...
/// 195종 풀에서 등급별 5/6/23/55/106(mythic→common) 분배.
/// 일반 가챠 종당 확률 ≈ Legendary 0.33% / Epic 0.35% / Rare 0.55% / Common 0.57%
/// (Mythic은 weight 0이라 일반 가챠 미등장 — 프리미엄 가챠권 전용 풀).
...
enum Gacha {
    ...
    nonisolated static let pool: [Rarity: [PetKind]] = [
        ...
        .common:    [.fox, .wolf, .bear, .boar, .deer, .rabbit,
                     ...
                     .scrapBot, .antennaBot, .spiderBot, .miniBot, .beaconBot,
                     .bushly, .rollingNero, .gloppySlime, .chiChiBird, .diverFish,
                     .bub, .spikeyBub, .pokeyBub, .blockyBub,
                     .octi, .roboPumpkin, .daikon, .rocketCherry, .cheesePuff, .snipCrab,
                     .gumBot, .twiggy,
