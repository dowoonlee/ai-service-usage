...
private struct GitHubLinkView: View {
    ...
    private func connectedView(login: String) -> some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            Text("머지된 기여 PR마다 \(CoinLedger.coinPerContributorPR) 코인 + \(RankPointLedger.rpPerContributorPR) RP 자동 적립.")
                .font(.system(size: 10))
                .foregroundStyle(.secondary)
            Button("지금 동기화") {
    ...
    private var disconnectedView: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            case .idle:
                ...
                Text("기여한 PR이 머지되면 \(CoinLedger.coinPerContributorPR) 코인 + \(RankPointLedger.rpPerContributorPR) RP가 자동 적립됩니다.")
                    .font(.system(size: 10))
                    .foregroundStyle(.secondary)
            case .requesting:
