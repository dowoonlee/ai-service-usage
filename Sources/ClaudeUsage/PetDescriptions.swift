...
enum PetDescriptions {
    ...
    static let perPet: [PetKind: String] = [
        ...
        // Superpowers 원시부족 — Deprecated 컬렉션 합류
        .caverman: "추상화 계층 없이 맨손으로 코드를 쪼던 원시인. 도구는 돌도끼(vim)뿐.",
        .caveGirl: "베리(dependency)를 채집해 오는 부족원. 뭐가 독인지는 먹어봐야 안다.",
        .caveGirl1: "동굴 벽에 첫 커밋 로그를 새기던 소녀. 벽화가 곧 changelog.",
        .dragonMan: "용의 피가 흐르는 부족 전사. 화날 때만 legacy 코드를 불태운다.",
        .lionWarrior: "사자 가죽을 쓴 부족 전사. prod 정글에서 가장 용맹하다.",
        .littleCaveBoy: "이제 막 첫 불(빌드)을 지핀 꼬마. 아직 손이 그을렸다.",
        // LuizMelo 히어로("10x Engineer")·몬스터(OOMKilled)·반려동물("Emotional Support")
        .heroKnight: "혼자서 던전 하나를 클리어하는 전설의 기사. PR 리뷰도 혼자 다 한다.",
        .huntress: "버그를 원거리에서 저격하는 사냥꾼. 화살 한 발이 곧 핫픽스.",
        .evilWizard: "금지된 매크로를 남발하는 흑마법사. 그의 코드는 아무도 못 읽는다.",
        .medievalKing: "코드베이스를 통치하는 군주. 그의 한마디가 곧 아키텍처 결정(ADR).",
        .martialHero: "맨손으로 레거시를 격파하는 무도가. 리팩터링이 곧 권법.",
        .fantasyWarrior: "대검으로 기술부채를 두 동강 내는 전사. 물러섬을 모른다.",
        .fireWorm: "불을 뿜으며 기어다니는 화염충. 로그를 통째로 태워버린다.",
        .vampireBat: "밤마다 커넥션을 빨아먹는 흡혈 박쥐. connection pool의 천적.",
        .mimic: "보물상자인 척하는 미믹. 클릭하는 순간 무한 루프에 갇힌다.",
        .giantRat: "하수구를 오가는 거대 쥐. 케이블(의존성)을 갉아먹는다.",
        .kingSlime: "분열하며 불어나는 킹 슬라임. 하나 죽이면 둘이 된다.",
        .cat1: "키보드 위에 눕는 게 특기. 그 위로 커밋이 지나간다.",
        .cat2: "까만 털의 야행성. 새벽 코딩의 유일한 목격자.",
        .cat3: "골골송으로 스트레스를 낮춰주는 반려묘. 무료 힐링 스택.",
        .cat4: "도도하지만 배포 성공하면 곁에 와 앉는다.",
        .cat5: "상자(container)만 보면 들어가는 습성. 격리가 취미.",
        .cat6: "고등어 무늬의 먹보. 간식(리워드) 앞에선 야근도 감수.",
        .akita: "충직한 아키타. 한 번 맡은 태스크는 끝까지 문다.",
        .goldenRetriever: "누구와도 페어링하는 골든. 온보딩 담당 멘토견.",
        .greatDane: "덩치 큰 데인. 서버 랙 옆을 지키는 든든한 감시견.",
        .saintBernard: "장애 현장에 구조통을 메고 달려오는 세인트 버나드.",
        .schnauzer: "수염이 멋진 슈나우저. 잔버그를 콧수염으로 감지한다.",
        .husky: "말 안 듣기로 유명한 허스키. config를 늘 자기 맘대로 바꾼다.",
    ]
