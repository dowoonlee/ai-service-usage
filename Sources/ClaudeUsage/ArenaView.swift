...
struct ArenaView: View {
    ...
    // 배틀 결과·팀 스냅샷 — 재생 자체(진행/연출 상태)는 BattleReplayView가 소유.
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 14) {
                ...
                if owned.isEmpty {
                    ...
                    .onChange(of: mode) { _, _ in result = nil; rankedResult = nil; rankedError = nil }
                    switch mode {
        ...
        .onDisappear { enhanceTask?.cancel(); rankedTask?.cancel() }
    }
    ...
    @ViewBuilder private var teamEditor: some View {
        HStack {
            ...
            Button { teamKinds = Array(owned.shuffled().prefix(5)); result = nil } label: {
    ...
    private func replayHistory(_ m: RankingAPI.PvpMatch) {
        rankedResult = nil; rankedError = nil
        ...
        // 재생은 BattleReplayView가 result 변화를 감지해 자동 시작.
    ...
    private func doRankedChallenge() {
        ...
        rankedError = nil; rankedResult = nil; result = nil
        let dev = settings.rankingDeviceID
    ...
    private func moveSlot(_ slot: Int, by delta: Int) {
        ...
        teamKinds.swapAt(slot, dst); result = nil
    }
    private func moveToLead(_ slot: Int) {
        guard teamKinds.indices.contains(slot), slot != 0 else { return }
        let k = teamKinds.remove(at: slot); teamKinds.insert(k, at: 0); result = nil
    }
    ...
    @ViewBuilder private var battleArena: some View {
        if let r = result {
            // 공유 재생 컴포넌트(BattleReplayView) — 도장 관장 배틀과 동일 연출. 랭크전 결과 카드는
            // resultExtra로 주입(재생 완료 시 배너 아래 노출).
            BattleReplayView(aSnaps: aSnaps, bSnaps: bSnaps, result: r,
                             serverMaxHpA: serverMaxHpA, serverMaxHpB: serverMaxHpB) {
                if let rr = rankedResult { rankedResultCard(rr) }
            }
        }
    }
    ...
    private func fight() {
        ...
        // 재생은 BattleReplayView가 result 변화를 감지해 자동 시작.
        result = BattleEngine.simulate(teamA: BattleTeam(aS), teamB: BattleTeam(bS), seed: .random(in: 1...UInt64.max))
    }
