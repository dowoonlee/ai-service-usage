...
final class NotificationManager {
    ...
    func rewardGrantEarned(currency: String, amount: Int, grantKey: String = "") {
        ...
        if grantKey.hasPrefix("pvp-season") {
            send(title: "🏆 아레나 시즌 보상",
                 body: "지난 시즌 순위 보상 +\(amount) \(unit)이 적립되었습니다!")
        } else {
            send(title: "🎁 보상 도착",
    ...
    func contributorBonus(prCount: Int, totalCoins: Int, totalRP: Int, prList: String) {
        let title = "기여자 보너스 +\(totalCoins) 코인 +\(totalRP) RP"
        let body  = prCount == 1
            ? "PR \(prList) 머지 감사합니다!"
