...
| 버전 | 날짜 | 주요 변경 |
|---|---|---|
| v0.2.0 | 2026-04-29 | **펫 가챠 컬렉션 시스템**. 펫 10→30종 (Pixel Adventure 2 enemies 18 + Rocks 3-variant 추가); 4단계 색상 변종(이로치) 5/15/40 중복 임계로 해금; 등급 분포 Common 60%/Rare 30%/Epic 8%/Legendary 2% (NinjaFrog 단독 Legendary); **사용량 비례 코인 적립** (Cursor cents, Claude pct delta) + 소수부 carry로 누락 방지; pullCost 자동 캘리브레이션 (avgDaily × 3.5, 일주일 2번 페이스); **포켓몬 스타일 알 부화** — 6회 탭 → 균열 점진 + 암전 + 흰 flash + 어두운 실루엣 + 색 fade in; 도감을 등급별로 분리(Legendary→Common) + 보유 펫 클릭 시 walking 미리보기; **휴식 권유 1시간 고정**(설정 토글 제거) + 1분 이내 클릭 시 +30 코인 + 코인 popping 파티클; 메인 패널에 코인/티켓 잔액 표시 |
...
| v0.1.9 | 2026-04-27 | 펫이 가끔 멈춰서 한국어 어처구니 한 마디를 7초간 말풍선 |
| v0.1.8 | 2026-04-27 | `.app/Contents/Resources/` 리소스 번들 미로드로 v0.1.7 launch 크래시 → resolver hotfix |
| v0.1.7 | 2026-04-27 | 차트 위 픽셀 펫 6종 + 사용량 mood + 테마 fill + 큰 낙폭 시 "AAAH!" (단 v0.1.7 zip은 위 크래시로 v0.1.8 사용) |
