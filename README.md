...
| 버전 | 날짜 | 주요 변경 |
|---|---|---|
| v0.1.11 | 2026-04-28 | Pixel Adventure 1 (CC0, Pixel Frog) 캐릭터 4종 추가 — 마스크 영웅, 닌자 개구리, 버섯, 슬라임. 선택 가능 펫 6 → 10. 두 sprite pack의 기본 향(좌/우)이 반대인 점은 `PetKind.defaultFacingLeft` 플래그로 처리. Claude 5시간 차트의 펫이 plot 좌/우 범위를 벗어나던 버그 수정 (펫이 받던 데이터가 차트가 그리는 데이터보다 넓었음 — 0% 스냅샷 포함 여부 mismatch). |
| v0.1.10 | 2026-04-27 | 큰 상승 segment 통과 시 점프 + "WHEE!" 환호 말풍선 추가 (굴러 떨어짐 + "AAAH!" 비명의 거울짝); 굴러떨어짐/점프 segment에서 펫이 1/1.5배 속도로 통과해 애니메이션·말풍선 1.5배 더 오래 지속; quote 말풍선이 텍스트 폭에 정확히 hug하고 plot 영역 안쪽으로 자동 클램프 (위로 잘릴 시 펫 아래로 뒤집힘) |
| v0.1.9 | 2026-04-27 | 펫이 가끔(~5%) 멈춰서 한국어 어처구니없는 한 마디를 7초간 말풍선으로 표시 |
