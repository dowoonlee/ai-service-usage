...
## ⚠️ 중요 — 비공식 endpoint 사용에 대한 고지

이 앱은 Claude.ai 와 Cursor 의 **비공식·문서화되지 않은 내부 API**를 사용해 사용량을 가져옵니다 (사용자 본인의 세션 쿠키/JWT 사용, read-only). Anthropic / Cursor 의 공식 API 가 아닙니다.

**알려진 위험**:
- **TOS 측면**: Anthropic Consumer Terms § 3 은 API Key 외 자동화된 access 를 일반적으로 금지합니다. Cursor TOS 도 비슷합니다. 글자 그대로 보면 **회색지대가 아니라 위반**일 수 있습니다.
- **계정 정지**: Anthropic 은 2026년 1월 OpenClaw 등 third-party harness 단속을 시작했고 (응답을 라우팅하는 도구 위주), read-only 사용량 트래커 그룹에는 아직 단속 보고가 없으나 **0 보장은 없습니다**.
- **endpoint 변경**: Anthropic/Cursor 가 endpoint 를 변경하거나 막으면 앱이 즉시 동작 정지합니다.

**v0.3.1 부터 적용된 완화**:
- Claude/Cursor 둘 다 실제 Safari User-Agent 사용 (이전 Cursor 측 `ClaudeUsage/1.0` 자체 UA 제거)
- 폴링 기본 600s (10분) 로 완화 (이전 300s)
- 폴링 간격에 ±15% jitter 적용 (정확히 일정한 robotic 패턴 회피)
- 429/5xx/네트워크 transient 에러 시 지수 backoff (최대 16배)
- macOS sleep/wake 시 폴링 일시중단 (깨자마자 폭주 방지)
- panel 닫혀있고 메뉴바 모드도 꺼진 상태에선 폴링 안 함

**그래도 위험을 감수하지 않으시려면 사용을 중단해주세요.** 작성자는 계정 차단·서비스 이용 정지 등의 결과에 대해 책임지지 않습니다.
...
| 버전 | 날짜 | 주요 변경 |
|---|---|---|
| v0.3.1 | 2026-04-29 | 비공식 endpoint 보호 — Cursor UA Safari로 통일; 폴링 기본 600s + ±15% jitter; 429/5xx 지수 backoff; sleep/wake/visibility gate; README disclaimer 추가 |
| v0.3.0 | 2026-04-29 | 펫 75종으로 확장(+45); 펫별 대사·설명; 사용 시간 기반 이로치 해금(4d/8d/12d); 도감 NEW! 강조 |
...
| v0.1.0 ~ v0.1.6 | 2026-04-24 ~ 27 | 첫 릴리스, 알림 임계치, 자동 시작, Sparkle 자동 업데이트 등 초기 인프라 |
