...
<p align="center">
  <img src="docs/images/main-panel.png" alt="AIUsage 메인 패널 — Claude·Cursor·Codex 사용량과 차트 위를 걷는 펫" width="320" />
</p>
...
- **코인 수급처**: 실제 AI 사용량 외에도 **사용 스트릭**(사용한 날 연속 카운트, 마일스톤 보너스)과 **오늘의 AI 뉴스 퀴즈**(최신 AI 뉴스 4지선다, 정답 수에 따라 코인) 등 반복 수급 경로 제공.
...
- **도장(Gym)**: 2개 대륙 9개 지역 × 19개 카테고리 × 4티어 = 76개 뱃지. 카테고리 진행률이 차면 **관장 배틀**이 열리고, 이기면 그 티어 뱃지를 획득. 지역 마스터·대륙 챔피언·그랜드 챔피언 보상.
- **펫 배틀 & 스킬**: 이로치 단계마다 스킬이 늘어난다(기본기 → 타입기 → 컬렉션기 → Epic+ 고유기), 레인보우는 **궁극기**까지. 6배틀타입 상성 + 자속 + 팀 시너지 + 패링·크리로 굴러가는 결정적 5v5 ATB 자동전투.
- **펫 강화**: +15까지 단계 강화. 상위 구간은 실패 시 유지/하락/파괴가 갈리고, 완화 아이템으로 확률을 조정.
...
- **아레나(랭크전)**: 5마리 팀을 등록해 두면 비슷한 레이팅의 상대와 **하루 10판** 매칭. 승패는 서버가 시뮬레이션해 확정하고(클라는 재생만) Elo로 정산되며, 시즌 종료 시 순위 보상. 상대를 고를 수 없고 결과를 위조할 수 없는 구조.
- **길드**: 길드를 창설하거나, 길드 리스트에서 가입신청(길드장 승인)·초대 코드·초대장으로 가입해 멤버 상위 5명의 VP 합산으로 길드 랭킹을 겨룸. 길드 사무실 씬 꾸미기, 길드장 초대장/가입신청 승인/추방/해체 관리.
...
- **오늘의 AI 뉴스 퀴즈**: 매일 갱신되는 AI 뉴스 브리핑 + 4지선다 3문항, 정답 수에 따라 코인 보상.
- **기여자 보상**: 이 레포에 PR이 머지되면 1건당 500 RP.

**기타**
- 패치 공지: 업데이트 후 첫 실행 시 새 버전 변경사항 창 표시, 확성기 버튼으로 다시 보기
- 인앱 사용 가이드: 책 아이콘 → 기능별 설명 17개 섹션
...
swift run ClaudeUsage --arena-demo   # 배틀·강화 엔진 헤드리스 실행
swift test                       # 테스트 (~220개, PR CI에서도 실행)
bash scripts/package.sh          # 릴리스 .app + .zip 생성 → dist/
```

> 개발 실행은 설치된 앱과 **같은** 로컬 데이터(`~/Library/Application Support/ClaudeUsage/`)를 씁니다.
> 실사용 기록을 건드리지 않으려면 `AIUSAGE_DATA_DIR=/tmp/demo swift run` 처럼 저장 위치를 따로 지정하세요.
> (이 README의 스크린샷도 그렇게 만든 예시 데이터입니다.)
