...
**Pet collections (set bonuses)** — 75 pets are partitioned into 11 dev-meme-themed `PetCollection` cases (`Works on My Machine` / `It's Always DNS` / ... / `--no-verify`). Collecting all base pets (variant 0) of a group credits a one-time bonus `Σ(member rarity coinValue) × 1.5` (Common=100 / Rare=300 / Epic=800 / Legendary=2500) and lights up an achievement card in the inventory's bottom section. Wired:
