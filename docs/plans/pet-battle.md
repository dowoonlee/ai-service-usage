...
- **턴 순서**: SPD 높은 쪽 선공, 동률은 시드로 결정(결정적).
- **데미지**: `dmg = (ATK / DEF) × movePower × typeEff × collectionMult × rand(0.9~1.0)`. ATK/DEF엔 이미 성장·팀 시너지가 반영됨(별도 levelScale 없음). `collectionMult`=펫간 상성(§2-10). `rand`는 **매치 시드 기반 결정적 PRNG**(서버·클라 동일). HP 0이면 교체.
...
- **승리**: 상대 3마리 전멸. 무한 루프 방지로 **최대 턴수 cap**(예: 50턴), 초과 시 잔여 HP 합 큰 쪽 승(결정적).
...
ENHANCE_VP_COST = [20,40,75,130,210,320,470,680,950,1300,1800,2500,3400,4600,6200]     // 지수 폭증
// 참고: 파괴 리셋 반영 +15 도달 기대 VP ≈ 5.3M (단순합 22.7k의 233×, 마르코프)
...
DAMAGE = (ATK/DEF) * movePower * typeEff * collectionMult * rng(0.9..1.0)  // ATK/DEF에 성장·팀시너지 반영
TURN_ORDER = SPD 내림차순, 동률은 시드; MAX_TURNS = 50 (초과 시 잔여 HP 합 큰 쪽)
...
- 강화·타입 동일이면 승률 ≈ 50% (대량 시드 표본). 항상 MAX_TURNS 이내 종료. 로그 데미지 ≥ 1.
