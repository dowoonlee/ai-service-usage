...
### A. 팀 시너지 (팀 빌딩 전략)
같은 컬렉션/타입 팀원끼리 스탯 버프. **동족=전 스탯 / 동타입=방향성(대표 스탯만)** 으로 차등.
- **같은 컬렉션**(강한 유대) 최대 동족: 2 → 전 스탯 +5% / 3 → +10%.
- **같은 타입**(느슨한 유대) 최대 동속 → **그 타입의 대표 스탯만** +3%(2)/+5%(3). 대표 스탯 = 아키타입
  성향: Beast/Chaos→SPD · Warrior/Arcane→ATK · Machine→DEF · Mascot→HP. 타입 정체성 강화 +
  "모노 Machine=방어벽" 류 팀빌딩 다양화.
- 모노 컬렉션(=동타입) 팀 예: 전 스탯 ×1.10 + 대표 스탯 추가 ×1.15 — 강하지만 단일 타입이라 카운터
  하나에 취약(리스크/리턴).
...
TEAM_SYNERGY = { collection:[2:.05,3:.10](전 스탯), type:[2:.03,3:.05](대표 스탯만 방향성) }
SIGNATURE_STAT = { beast:spd, chaos:spd, warrior:atk, arcane:atk, machine:def, mascot:hp }  // 타입 대표 스탯
