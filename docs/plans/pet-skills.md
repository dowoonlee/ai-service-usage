...
## 5. 궁극기 메커닉 — 팀 게이지 (Phase C 구현 → 가시성 패치로 개정)
- 게이지 충전(전부 이벤트 기반 = **RNG 불필요·완전 결정적**, 파리티 안전): ①**행동 시 +1** ②**피격 시 +1**(막타 포함) ③**기절 시 잔여 게이지를 다음 생존 펫에게 승계**(개인 게이지 → 사실상 팀 게이지). **`ultChargeCost=10`** 도달 시 그 행동이 궁극기(정규 스킬 대체), 게이지 리셋 → 장기전 다회 발동.
- **가시성 패치 근거(실측 500판×3시나리오, 양팀 전원 v4)**: 구 규칙(행동만 +1·개인 게이지·N=6)은 패자 펫이 펫당 1.5~3.3행동 만에 죽어 게이지가 안 참 → 패자 궁 발동률 동급 62% / 소폭 우위 61% / 대폭 우위 **14%**. 피격 충전은 지는 쪽도 맞으면서 차게 하고(양측 충전 속도 대칭 — 격투게임 미터 방식), 승계는 죽음으로 게이지가 증발하는 걸 막는다 → **100/100/91%**. 판당 궁극기 2.8→5.8(사이드당 ~2.9, 5마리 팀 기준 펫당 ~0.6회), **승률·라운드 수 불변**(가시성 전용 — 스노우볼 밸런스와 독립). N=12(4.8개/판, 대폭 88%)도 후보였으나 팀 5마리 규모 대비 N=10의 "사실상 보장"을 채택.
- 한계: 레어리티+시너지+강화가 전부 겹친 극단 학살전(~5행동 종결)은 여전히 아무도 게이지에 못 닿음 — 배틀 자체가 스킬 몇 방으로 끝나는 케이스라 허용.
...
- **가시성 패치** ✅ — 충전을 팀 게이지(행동+피격+승계, `ultChargeCost=10`)로 개정 — §5. 패자 측 궁 발동률 62/61/14% → 100/100/91%.
- **Phase D**(선택) — 스킬 도감 UI(타입 상성표 원형 옆에), 밸런스 튜닝(승률 실측), 충전 게이지 시각화(현재 로그 태그뿐).
