...
- **E4** ✅ UI: 전투원 상태 아이콘·스킵 연출·효과 로그 확장(ArenaView 전용, 엔진·파리티 무관). **구현 노트 §8.7.**
  이후 승률 재실측 → 밸런스 재평가.
...
## 8.7 구현 노트 (E4 — 상태 시각화 UI, ArenaView 전용)
- **효과 상태 재구성**: `battleDicts`가 HP·게이지에 더해 각 펫의 활성 효과(id + 남은 자기 턴)까지 접는다.
  remaining 카운트다운은 **"자기 턴" 축**(공격자·스킵·DoT 자멸 라운드 = 그 펫의 tick) — 엔진 tickEffects와
  동일 축이라 statMod/control까지 만료가 근사 재현된다. slot cap(4)은 표시상 무시(칩도 최대 4개). cleanse
  grant는 그 펫 디버프 제거. **순수 재생 — 파리티 무관, effectEvents 없는 구 로그도 안전.**
- **상태 칩**: hpBox 하단에 효과 칩(이모지+남은 턴, 디버프 빨강/버프 초록, .help 툴팁). 이모지는 id 매핑 +
  EffectKind 폴백(미래 효과 자동 커버).
- **스킵 연출**: 스킵 라운드는 공격 로그가 없어 자체 재생 스텝이 없다 → "직전 공격 스텝 라운드와 현재
  스텝 라운드 사이"의 skip 이벤트를 현재 스텝에 얹어 💤 말풍선. 재생 루프는 무변경(안전).
- **로그 확장**: `fullLog`가 공격 이벤트 + 스킵/틱 효과 이벤트를 라운드 순서로 병합(스킵 이유·턴시작
  지속피해/회복이 보임). grant/heal/splash는 공격 줄의 effectLine과 중복이라 로그에선 스킵·틱만.
