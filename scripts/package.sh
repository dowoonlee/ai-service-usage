...
# scripts/ranking.env — 랭킹 시스템 환경변수 (gitignored). 없으면 랭킹 UI 자동 비활성.
# CI에선 secrets로 export하므로 이 파일은 로컬 빌드 전용.
...
# 랭킹 시스템 — Supabase + 처리방침 URL. 셋 다 공개 OK (anon key는 RLS로 권한 통제).
# 셋 중 하나라도 비어 있으면 클라이언트의 RankingAPI.isConfigured == false → UI 비활성.
...
<dict>
    ...
    <key>SupabaseURL</key><string>${SUPABASE_URL}</string>
    <key>SupabaseAnonKey</key><string>${SUPABASE_ANON_KEY}</string>
    <key>PrivacyPolicyURL</key><string>${PRIVACY_POLICY_URL}</string>
</dict>
</plist>
PLIST
...
if [ -z "${SUPABASE_URL}" ] || [ -z "${SUPABASE_ANON_KEY}" ]; then
  echo "NOTE: SUPABASE_URL/SUPABASE_ANON_KEY 미설정 — 랭킹 기능이 이 빌드에서 비활성화됩니다." >&2
fi
